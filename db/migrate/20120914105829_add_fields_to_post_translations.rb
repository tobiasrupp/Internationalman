  ...
  def self.up
    ...
    # migrate existing post data to translation table
		...
		Post.all.each do |post|
			if post.text
		  execute "UPDATE post_translations SET text=\"#{raw post.text}\" WHERE post_ID='#{post.id}'"
			end
			if post.text_2
		  execute "UPDATE post_translations SET text_2=\"#{raw post.text_2}\" WHERE post_ID='#{post.id}'"
			end
		end
  end
